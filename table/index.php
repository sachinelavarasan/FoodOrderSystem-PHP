...
<center>
      <h3 style="color:green;font-weight: bold;">Customer Login</h3>
...
if(isset($_POST['submit'])){
        ...
        $cekuser = mysqli_query($conn,"SELECT * FROM tablelogin WHERE tabno = '$username'");
        $ro= mysqli_num_rows($cekuser);
        
        if($ro>0) 
        {
          $check= "SELECT * FROM tablelogin WHERE tabno = '$username' and password = '$password'";
 ...
 echo "<script>alert('Your Account has Login! $username'); window.location = 'nextpage.php'</script>";
}
}else{
